...
const Register = () => {
    ...
    axios
      ...
      .catch((error) => {
        if (error.response) {
          ...
          alert(
            error.response.data.message || "서버 응답 중 오류가 발생하였습니다"
          );
  ...
  const userPassword = register("userPassword", {
    ...
    pattern: {
      value:
        /^(?=(?:.*[a-z])(?:.*[A-Z])|(?:.*[a-z])(?:.*[0-9])|(?:.*[A-Z])(?:.*[0-9]))[a-zA-Z0-9]+$/,
  ...
  return (
    <div className="Register" style={{ margin: "0 auto", maxWidth: "1280px" }}>
      <Header />
      <div
        style={{ margin: "0 auto", padding: "58px 0 160px", width: "600px" }}
      >
        <h1
          className="logo_area"
        ...
        >
          <img
            src={process.env.PUBLIC_URL + `assets/lettersLogo.png`}
            alt="bluewave"
            className="letters_logo"
        ...
        <form noValidate onSubmit={handleSubmit(onSubmit)}>
          ...
          <Input
            ...
            title={"비밀번호"}
            placeholder={
              "영문 소문자, 영문 대문자, 숫자 중 2가지 이상 조합  8-16자"
            }
          />
          <Input
            type={"password"}
            id={"comparePassword"}
            prop={comparePassword}
            errors={errors}
            title={"비밀번호 확인"}
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <p style={{ width: "calc(100% - 40px)", textAlign: "left" }}>
                주소 검색
              </p>
              <img
          ...
          <div className="input_box">
            <div className="input_item">
              <input
                ...
                placeholder="상세주소"
                id="detailAddress"
                className={errors.detailAddress ? "input-error" : "input_txt"}
                ref={addrDetailRef}
            ...
            {errors.detailAddress && (
              <p className="errorTxt">{errors.detailAddress.message}</p>
            )}
          </div>
          ...
          <Button
            text={"가입하기"}
            className={
              isValid && allChecked ? "wide_btn pass_btn" : "wide_btn fail_btn"
            }
            disabled={!isValid || !allChecked || isSubmitting}
            type="submit"
          />
        </form>
      </div>
